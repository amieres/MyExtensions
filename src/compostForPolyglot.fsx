...
module MySvg =
    ...
    let makeElement name (attrs: HtmlAttribute list) (content: Object list) = 
        ...
        (HtmlElement.Tagged p :> Microsoft.AspNetCore.Html.IHtmlContent)        

    type SvgHtml() = 
        let a = 1
        static member (?) (this: SvgHtml, name:string) =
            fun (attrs:list<string * _ >) (content:list<Object>) ->
                makeElement name (attrs |> List.map (fun (a,b) -> a,(b:>obj)) ) content

    let s = SvgHtml()

    let circle (cx:int) (cy:int) (r:int) = 
        s?circle [
            "cx"          , $"{cx}"
            "cy"          , $"{cy}"
            "r"           , $"{r}"
    ...
    let myCircle = circle 50 50 30

    let canvas (w:int) (h:int) style = 
        s?svg [ _width $"{w}" ; _height $"{h}" ; style ] 

    let result = 
        s?div [] [
            "Hello"
            div [] []
            canvas 100 100 (_style [ "background-color:blue" ]) [ 
                myCircle
            ]
        ]
...
module Svg =
        ...
        type RenderingContext = { Definitions : ResizeArray<Microsoft.AspNetCore.Html.IHtmlContent> }
...
module Scales = 
        ...
        let generateAxisSteps s =
            match s with 
            | Continuous(CO l, CO h) -> generateSteps 6. 1. (float l, float h) |> Array.map (fun f -> COV(CO (unbox f)))
...
module Drawing = 
        ...
        type DrawingContext = {
            Style : Style
            Definitions : ResizeArray<Microsoft.AspNetCore.Html.IHtmlContent> 
        }

        let rec hideFill   (style:Style) = 
            { style with Fill = Solid(0.0, RGB(0, 0, 0)); Animation = match style.Animation with Some(n,e,f) -> Some(n,e,f >> hideFill) | _ -> None }
        let rec hideStroke (style:Style) = 
        ...
        let rec drawShape ctx ((x1, y1, x2, y2) as area) ((sx, sy) as scales) (shape:ScaledShape<'ux, 'uy>) =     
            ...
            | ScaledOffset((dx, dy), shape) -> drawShape ctx (x1 + dx, y1 + dy, x2 + dx, y2 + dy) scales shape
            | ScaledLayered          shapes -> Combine(Array.map (drawShape ctx area scales) shapes)
            | ScaledStyle(f,         shape) -> drawShape { ctx with Style = f ctx.Style } area scales shape
            ...
            //| ScaledInteractive(f, _, _, shape) -> drawShape ctx area scales shape
...
module Compost =
        ...
        let cov x   = x |> CO |> COV
        let car x y = CAR(CA x , y)        

        let line         pts   = Line pts
        let column       ca co = Column(     CA ca, CO co)
        let bar          co ca = Bar(        CO co, CA ca)
        let fillColor    c   o = FillColor(  c    ,     o)
        let strokeColor  c   o = StrokeColor(c    ,     o)
        let overlay          x = Shape.Layered x
        let axes  (a:string) o = Shape.Axes(a.Contains("top"), a.Contains("right"), a.Contains("bottom"), a.Contains("left"), o)
        let scaleX       sc sh = Shape.InnerScale(Some(sc), None, sh) 
        let scaleY       sc sh = Shape.InnerScale(None, Some(sc), sh)
        let continuous   lo hi = Continuous(CO lo, CO hi)
        let padding t r b l sh = Shape.Padding((t, r, b, l), sh)
        let font        f c sh = Derived.Font(f, c, sh)
        let nest lx hx ly hy s = Shape.NestY(cov ly, cov hy, Shape.NestX(cov lx, cov hx, s))
        let text  xp  yp t (s:string) r = 
            ...
            let ha = if s.Contains "start"    then HorizontalAlign.Start elif s.Contains "end"     then End     else Center
...
module Test =
    ...
    let rectangle xf xt yf yt = //:Shape<1,1> =
        Line [
            (cov xf , cov <| 100. - yf)
            (cov xt , cov <| 100. - yf)
            (cov xt , cov <| 100. - yt)
    ...
    let inline errorLine x y sdv =
        [
            line [ car x 0.50 , y - sdv |> cov ; car x 0.50 , y + sdv |> cov ]
            line [ car x 0.45 , y - sdv |> cov ; car x 0.55 , y - sdv |> cov ]
            line [ car x 0.45 , y + sdv |> cov ; car x 0.55 , y + sdv |> cov ]
        ]
        |> overlay
        |> strokeColor "black"

    let columnWithDev x y sdv =
        [
            yield column    x y 
            yield errorLine x y sdv
        ]

    let legend data : Shape<1,1>  =
        let sz = 35.
        [
            [
                for (nm, v, sdv, color) in data do 
                    yield bar sz nm |> padding 3 5 3 5 |> fillColor color
                yield
                    [
                        for (nm, v, sdv, color) in data ->
                            text (cov sz) (car nm 0.5) nm "start" 0.
                    ]
                    |> overlay
                    |> font "9pt arial" "black"
            ]
            |> overlay
            |> scaleX (continuous 0. 100. )
            |> nest   0. 100.  0. 100.
            rectangle 0. 100. 0. 100. |> strokeColor "white"
        ]
        |> overlay
    ...
    let chart data = 
        [
            for (nm, v, sdv, color) in data do
                for x in columnWithDev nm v sdv do
                    yield x |> fillColor color
        ]
        |> overlay
        |> scaleY (continuous 0. 100.)
        |> axes "left bottom"

    let title :Shape<1,1> =
        text (cov 50.) (cov 5.)  "Chart" "center top" 0.
        |> font "15pt arial" "white"
        |> scaleX (continuous 0. 100.)
        |> scaleY (continuous 0. 100.)

    let demo data = 
        [
            nest  0.  95.  0.  90. (chart  data)
            nest 85.  98. 75.  95. (legend data)
            nest  0. 100.  0. 100. title
        ] 
        |> overlay
    ...
    let render w h st x = 
        div [ _style (List.append st [ $"width:{w}px" ; $"height:{h}px" ]) ] [ 
            createSvg (w, h) x
        ]

    let result = demo data |> render 640 480 ["background-color: #aaa"]
